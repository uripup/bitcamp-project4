...
public class ListUserDao implements UserDao {
    ...
    try (XSSFWorkbook workbook = new XSSFWorkbook(path)) {
      ...
      for (int i = 1; i <= sheet.getLastRowNum(); i++) {
        ...
        try {
          ...
          user.setID(row.getCell(2).getStringCellValue());
          user.setPassword(row.getCell(3).getStringCellValue());
          user.setPoint(Integer.parseInt(row.getCell(4).getStringCellValue()));
  ...
  public void save() throws Exception {
    try (FileInputStream in = new FileInputStream(path);
      ...
      // 셀 이름 출력
      String[] cellHeaders = {"no", "name", "ID", "password", "point"};
      ...
      for (User user : userList) {
        ...
        dataRow.createCell(2).setCellValue(user.getID());
        dataRow.createCell(3).setCellValue(user.getPassword());
        dataRow.createCell(4).setCellValue(String.valueOf(user.getPoint()));
      }
